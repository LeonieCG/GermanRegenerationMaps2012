...
# dir.create(paste0("output/Fits/Basalarea/",outloc))
# for (i in species.vect) {dir.create(paste0("output/Fits/Basalarea/",outloc,i))} # creates folder structure
...
pdf(paste0("output/Fits/Basalarea/",outloc,"Basalarea_DHARMaresidual.pdf"), width = 10, height = 10) # all graphs will be printed here
...
for(species in species.vect) {
  print(species)
  try({
    fit  <-  readRDS(paste0("output/Fits/Basalarea/",outloc,species,"/Basalarea_",species,"_fit.rds"))
...
## Save outputs -------------------------------------------------------------
    ...
    write.csv(df.out, paste0("output/Fits/Basalarea/",outloc,species,"/Basalarea_",species,"_fit.csv"), row.names = F)
...
# Save df
write.csv2(df.allsp, paste0("output/Fits/Basalarea/",outloc,"Basalarea_model_summary.csv"), row.names = FALSE)
